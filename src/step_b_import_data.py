...
class DataLoader:
    ...
    def save_versions(self, dataframe=None, version=None):
        ...
        new_path = "/Users/lawhea1214/Documents/WGU/602/lorraine-w_task2/data/backup"
...
class FileLoader:
    def __init__(self, url, filename, filetype=''):
      self.url = url
      self.filename = filename
      self.filetype = filetype
      if filetype != '':
          self.filepath = os.path.join(url, f"{filename}.{filetype}")
      else: 
          self.filepath = os.path.join(url,filename)


    def create_it(self, entry, modetype):
        with open(self.filepath, mode=modetype) as doc:
            doc.write(entry)

    def read_it(self, modetype):
        with open(self.filepath, mode=modetype) as doc:
            doc.readlines(entry)
